...
void DirectDetectorExample() {
    ...
    keypoints[LEFT_EYE*2] = 110; keypoints[LEFT_EYE*2+1] = 75;
    keypoints[RIGHT_EYE*2] = 130; keypoints[RIGHT_EYE*2+1] = 75;
    keypoints[LEFT_SHOULDER*2] = 90; keypoints[LEFT_SHOULDER*2+1] = 140;
    keypoints[RIGHT_SHOULDER*2] = 150; keypoints[RIGHT_SHOULDER*2+1] = 140;
...
void DrawingExample() {
    ...
    keypoints[LEFT_SHOULDER*2] = 90; keypoints[LEFT_SHOULDER*2+1] = 140;
    keypoints[RIGHT_SHOULDER*2] = 150; keypoints[RIGHT_SHOULDER*2+1] = 140;
    keypoints[LEFT_WRIST*2] = 60; keypoints[LEFT_WRIST*2+1] = 200;
    keypoints[RIGHT_WRIST*2] = 180; keypoints[RIGHT_WRIST*2+1] = 200;
 ...
 * 这个函数应该在系统启动时调用
 */
bool InitializePostureDetectionSystem() {
    ESP_LOGI(TAG, "正在初始化坐姿检测系统...");
    
#ifdef CONFIG_IDF_TARGET_ESP32P4
    // 初始化COCO姿态检测模型
    if (!COCOPoseModelManager::GetInstance().InitializeModel()) {
        ESP_LOGE(TAG, "初始化COCO姿态检测模型失败");
        return false;
    }
    
    ESP_LOGI(TAG, "坐姿检测系统初始化成功");
    return true;
#else
    ESP_LOGW(TAG, "当前芯片不支持坐姿检测功能");
    return false;
#endif
}
...
void RunPostureDetectionExamples() {
    ...
    // 首先初始化坐姿检测系统
    if (!InitializePostureDetectionSystem()) {
        ESP_LOGE(TAG, "坐姿检测系统初始化失败，停止示例演示");
        return;
    }
